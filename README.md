...
Internal architecture note: runtime orchestration helpers live under `mi/runtime/autopilot/` (including a state-machine/contracts layer in `state_machine.py` + `contracts.py`, workflow cursor helpers, workflow-progress query/event/persist helpers in `workflow_progress_flow.py`, evidence-event append/window/segment side-effect helpers in `evidence_flow.py`, user-input/auto-answer record write helpers in `interaction_record_flow.py`, claim-mining helpers in `claim_mining_flow.py`, risk-event append/window/segment side-effect helpers in `risk_event_flow.py`, auto-answer query/fallback normalization helpers in `auto_answer_flow.py`, mind-call/circuit-break helpers in `mind_call_flow.py`, cross-project recall write-through helpers in `recall_flow.py`, next-input queue + loop-guard/loop-break helpers in `next_input_flow.py` and loop-break checks input helpers in `loop_break_checks_flow.py`, ask-user branch orchestration + re-decide-after-user helpers in `ask_user_flow.py`, pre-decide user-interaction/retry helpers in `predecide_user_flow.py`, decide-next prompt query/record side-effect helpers in `decide_query_flow.py`, check-plan query/record helpers in `check_plan_flow.py`, canonical testless strategy sync/write and TLS-resolution/replan helpers in `testless_strategy_flow.py`, batch context/effects helpers with shared context construction in `batch_context.py` (`build_batch_execution_context`), pre-decide pipeline helpers, reusable phase helpers for evidence/risk policy, orchestration service hooks under `mi/runtime/autopilot/services/` including pipeline/decide/checkpoint wrappers, and run-end `learn_update` + WhyTrace flows). Run wiring is now centered on `RunSession` (`run_context.py`) + `RunLoopOrchestrator` (`orchestrator.py`): `run_autopilot` remains behavior-preserving but delegates loop/pipeline/checkpoint composition to this orchestrator, while each single batch still flows through explicit pre-decide sub-phases (`run_hands` + preaction arbitration helper) and a dedicated decide phase helper; runner-side helper dependency wiring is centralized via local `_mk_*_deps` constructors to reduce branch drift. CLI parsing is composed in `mi/cli_parser.py` + `mi/cli_parsers/` (argument surface) while execution/dispatch lives in `mi/cli_dispatch.py` + `mi/cli_commands/` (`show`/`tail`, domain handlers, runtime handlers for `run`/`memory`/`gc`, status/project-selection handlers, and config/init/values/settings handlers). Values writing logic is additionally extracted into `mi/cli_commands/values_set_flow.py` (`run_values_set_flow`) and injected by `cli_dispatch` for reuse/testing. Thought DB internals are layered as append/view/service components (`mi/thoughtdb/append_store.py`, `mi/thoughtdb/view_store.py`, `mi/thoughtdb/service_store.py`) behind `ThoughtDbStore`, with a shared application-level facade in `mi/thoughtdb/app_service.py` used by runner and CLI paths (`show`/`workflow`/`claim`/`node`/`why`) and run-end WhyTrace candidate assembly for effective lookup, subgraph building, WhyTrace candidate flow, and decide-context building. Provider wiring is modularized via `mi/providers/mind_registry.py` + `mi/providers/hands_registry.py` (re-exported by `mi/providers/provider_factory.py`). Shared Mind provider helpers live under `mi/providers/mind_utils.py` (including filename-safe transcript stamping via `mi/core/storage.py` `filename_safe_ts`). Codex Hands runner lives under `mi/providers/codex_runner.py`; shared subprocess stream -> transcript plumbing lives under `mi/providers/proc_stream.py`; shared interrupt utilities live under `mi/providers/interrupts.py`. Host workspace adapters (derived artifacts) live under `mi/workflows/host_adapters/` (registry: `mi/workflows/host_adapters/registry.py`) with orchestration in `mi/workflows/hosts.py`. This is behavior-preserving and keeps public CLI/storage contracts unchanged.
...
# Shorthands:
mi last --cd /path/to/your/project --json          # == mi show last ...
mi ev_<id> --cd /path/to/your/project --json       # == mi show ev_<id> ...
mi hands --cd /path/to/your/project -n 200         # == mi tail hands ...
mi mind --cd /path/to/your/project -n 200          # == mi tail mind ...
