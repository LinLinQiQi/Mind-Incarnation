...
内部架构说明：runtime 编排辅助逻辑已拆分到 `mi/runtime/autopilot/`（包含 run-end WhyTrace 流程），CLI 处理已拆分到 `mi/cli_commands/`（`show`/`tail` 以及知识库与 workflow/host 路由）。该改动保持行为不变，不改变公开 CLI/存储契约。
