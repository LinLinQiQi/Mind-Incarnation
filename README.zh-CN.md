...
内部架构说明：runtime 编排辅助逻辑位于 `mi/runtime/autopilot/`（包含 `state_machine.py` + `contracts.py` 的状态机/契约层、workflow cursor 辅助逻辑、`workflow_progress_flow.py` 中的 workflow-progress 查询/记录/持久化 helper、`evidence_flow.py` 中的 evidence 追加/窗口写入/segment 同步 helper、`interaction_record_flow.py` 中的 user_input/auto_answer 记录落盘 helper、`claim_mining_flow.py` 中的 claim mining helper、`risk_event_flow.py` 中的 risk_event 追加/窗口写入/segment 同步 helper、`auto_answer_flow.py` 中的 auto-answer 查询/回退归一化 helper、`mind_call_flow.py` 中的 mind-call/circuit-break helper、`recall_flow.py` 中的 cross-project recall 落盘/write-through helper、`next_input_flow.py` 中的 next-input queue + loop-guard/loop-break helper、`loop_break_checks_flow.py` 中的 loop-break checks input helper、`ask_user_flow.py` 中的 ask-user 分支编排 + 用户回答后 re-decide helper、`predecide_user_flow.py` 中的 pre-decide 用户交互/重试 helper、`decide_query_flow.py` 中的 decide-next 提示查询/记录副作用 helper、`check_plan_flow.py` 中的 check-plan 查询/记录 helper、`testless_strategy_flow.py` 中的 testless 策略同步/写入 + TLS 解析/重规划 helper、`batch_context.py` 中的 batch context 构建 helper（`build_batch_execution_context`）、batch context/effects 辅助组件、pre-decide pipeline 辅助模块、evidence/risk 策略 phase helper、`mi/runtime/autopilot/services/` 下的编排 service hooks（含 pipeline/decide/checkpoint wrappers），以及 run-end `learn_update` + WhyTrace 流程）。运行期编排现已收敛到 `RunSession`（`run_context.py`）+ `RunLoopOrchestrator`（`orchestrator.py`）：`run_autopilot` 仍保持行为不变，但把 loop/pipeline/checkpoint 的组合交给 orchestrator；每个 batch 依旧走“pre-decide 子阶段（run_hands + preaction arbitration helper）+ decide phase helper”，并且 runner 内的 helper 依赖组装已通过本地 `_mk_*_deps` 构造器收敛，减少分支漂移。CLI 的参数解析/表面由 `mi/cli_parser.py` + `mi/cli_parsers/` 组合生成，而执行/dispatch 位于 `mi/cli_dispatch.py` + `mi/cli_commands/`（`show`/`tail`、领域路由、`run`/`memory`/`gc` 运行时命令处理、status/project-selection 处理，以及 config/init/values/settings 处理）。values 写入逻辑进一步抽离到 `mi/cli_commands/values_set_flow.py`（`run_values_set_flow`），由 `cli_dispatch` 注入调用，便于复用与测试。Thought DB 内部以 append/view/service 三层实现（`mi/thoughtdb/append_store.py`、`mi/thoughtdb/view_store.py`、`mi/thoughtdb/service_store.py`），并通过 `ThoughtDbStore` 对外保持一致；同时新增应用层门面 `mi/thoughtdb/app_service.py`，由 runner、CLI（`show`/`workflow`/`claim`/`node`/`why`）以及 run-end WhyTrace 候选组装统一调用，用于 effective 查询、subgraph 构建、WhyTrace 候选与 decide-context 构建。providers 侧 wiring 已抽离到 `mi/providers/mind_registry.py` + `mi/providers/hands_registry.py`（由 `mi/providers/provider_factory.py` 统一 re-export）。Mind provider 的通用 helper（schema 路径解析、JSON 提取、JSONL transcript 追加、转录文件命名）位于 `mi/providers/mind_utils.py`（其中转录文件命名使用 `mi/core/storage.py` 的 `filename_safe_ts`）。Codex Hands runner 位于 `mi/providers/codex_runner.py`；共享的 subprocess stream -> transcript plumbing 位于 `mi/providers/proc_stream.py`；共享的 interrupt 工具位于 `mi/providers/interrupts.py`。宿主 workspace 的 adapter（派生物写入）位于 `mi/workflows/host_adapters/`（registry: `mi/workflows/host_adapters/registry.py`），并由 `mi/workflows/hosts.py` 统一编排。该改动保持行为不变，不改变公开 CLI/存储契约。
...
# 更短写法：
mi last --cd /path/to/your/project --json      # 等价：mi show last ...
mi ev_<id> --cd /path/to/your/project --json   # 等价：mi show ev_<id> ...
mi hands --cd /path/to/your/project -n 200     # 等价：mi tail hands ...
mi mind --cd /path/to/your/project -n 200      # 等价：mi tail mind ...
...
注：`mi show last --json` 在存在 WhyTrace（例如通过 `mi run --why`）时，会包含 `why_trace` / `why_traces`；当 MI 需要 quarantine 损坏的 state 文件时，会包含 `state_corrupt_recent`；在 MI 检测到并尝试打破“重复卡住”的循环时，会包含 `loop_guard` 和 `loop_break` 字段。你也可以通过 `MI_STATE_WARNINGS_STDERR=1`（强制打印）/ `0`（静默）控制底层 state 告警的 stderr 输出。
...
mi /path/to/your/project status             # 等价：mi -C /path/to/your/project status
```
