...
- Internal implementation note: orchestration helpers are modularized under `mi/runtime/autopilot/` (including run-end `learn_update` + WhyTrace flows), and CLI handlers are modularized under `mi/cli_commands/` (`show`/`tail` plus domain handlers for claim/node/edge/why/workflow/host); this is behavior-preserving and does not change CLI/storage contracts.
