...
Internal wiring (behavior-preserving): `mi/runtime/runner.py:run_autopilot` is the public entrypoint (thin wrapper). `mi/runtime/runner_core.py:run_autopilot` delegates bootstrap (config/overlay/session initialization + workflow trigger injection) to `mi/runtime/wiring/bootstrap.py`, then delegates post-boot wiring + run-loop composition to `mi/runtime/runner_wiring_builder.py:run_autopilot_from_boot`. Run-level mutable state lives in `mi/runtime/runner_state.py` (`RunnerWiringState` + `RunnerStateAccess`) to reduce wiring drift (instead of many `nonlocal` vars / setter closures). Prompt builders live in `mi/runtime/prompts/` and are imported as `mi.runtime.prompts`. It runs run-start seeding via `mi/runtime/wiring/run_start.py`, parses runtime feature flags via `mi/runtime/wiring/runtime_cfg.py`, wires check planning via `mi/runtime/wiring/check_plan.py`, wires auto_answer_to_hands prompt/query plumbing via `mi/runtime/wiring/auto_answer.py`, wires ask-user auto-answer/redecide plumbing via `mi/runtime/wiring/ask_user.py`, wires interaction record plumbing (user_input/auto_answer evidence) via `mi/runtime/wiring/interaction_record.py` (with runner wiring bundled in `mi/runtime/runner_wiring_interaction.py`), wires evidence record plumbing (extract_evidence EvidenceLog append) via `mi/runtime/wiring/evidence_record.py`, bundles batch execution context construction (prompt sha/injection/resume metadata) in `mi/runtime/runner_wiring_batch_context.py`, bundles Hands execution wiring (one batch run + overlay persistence) in `mi/runtime/runner_wiring_hands.py`, bundles predecide wiring (extract_evidence/workflow_progress/checks/auto_answer) in `mi/runtime/runner_wiring_predecide.py`, bundles workflow+risk phase wiring (workflow_progress + risk gate callback set) in `mi/runtime/runner_wiring_workflow_risk.py`, bundles preaction arbitration wiring (needs-user + testless resolution + queue checks/answer) in `mi/runtime/runner_wiring_preaction.py`, bundles decide_next + ask_user orchestration wiring in `mi/runtime/runner_wiring_decide.py`, bundles loop-guard/loop-break next-input queueing wiring (including loop-break checks input computation) in `mi/runtime/runner_wiring_next_input.py`, wires pre-decide needs-user subflows (auto-answer -> recall -> prompt -> queue) via `mi/runtime/wiring/predecide_user.py`, wires workflow_progress query/apply plumbing via `mi/runtime/wiring/workflow_progress.py`, wires risk_judge query/record plumbing via `mi/runtime/wiring/risk_predecide.py` (with runner wiring bundled in `mi/runtime/runner_wiring_risk.py`), wires decide_next prompt/query + record-effects plumbing via `mi/runtime/wiring/decide_next.py`, wires testless-strategy canonicalization/resolution via `mi/runtime/wiring/testless_strategy.py` (with builder-local wiring bundled in `mi/runtime/runner_wiring_testless.py`), wires loop-break check computation via `mi/runtime/wiring/loop_break_checks.py`, wires checkpoint pipeline composition via `mi/runtime/wiring/checkpoints.py` (with runner wiring bundled in `mi/runtime/runner_wiring_checkpoint.py`), wires checkpoint mining/materialization callbacks via `mi/runtime/wiring/mining.py`, wires next-input queueing core flow via `mi/runtime/wiring/next_input.py`, bootstraps segment buffer IO via `mi/runtime/wiring/segments.py`, centralizes Mind call circuit-break state via `mi/runtime/wiring/mind_call.py`, centralizes state-corruption warning flushing via `mi/runtime/wiring/state_warnings.py`, builds the run-loop orchestrator in `mi/runtime/composition.py`, and runs it via `RunLoopOrchestrator` (`mi/runtime/autopilot/orchestrator.py`). Small runner-only pure helpers live in `mi/runtime/runner_helpers.py` (best-effort coercions like `dict_or_empty`, and check-plan extraction like `get_check_input`). Thought DB prompt-context assembly and deterministic retrieval helpers are routed through `ThoughtDbApplicationService` (`mi/thoughtdb/app_service.py`) and shared building blocks in `mi/thoughtdb/predicates.py` (active/valid filters) + `mi/thoughtdb/retrieval.py` (seed ids + 1-hop expansion). CLI entrypoint remains `mi/cli.py:main` (argparse surface composed in `mi/cli_parser.py` + `mi/cli_parsers/`, behavior-preserving). Phase semantics and user-visible loop behavior remain unchanged.
...
- `hands.provider=codex` (default)
  ...
  - Implementation note (behavior-preserving): shared subprocess stream -> transcript plumbing (stdout/stderr multiplex, interrupt signal escalation, optional live tee with `--redact`) is centralized in `mi/providers/proc_stream.py` and reused by both Codex and generic CLI Hands providers.
...
- `mind.provider=openai_compatible`
  ...
  - Response shape requirement: MI expects `choices[0].message.content` (string) to contain the JSON output. Other payload shapes (for example, `choices[0].text` or the Responses API) are not supported.
...
- `mind.provider=anthropic`
  ...
  - Response shape requirement: MI expects `content[]` text blocks (Messages API). `completion` payloads are not supported.
