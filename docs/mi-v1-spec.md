...
Internal wiring (behavior-preserving): `mi/runtime/runner.py:run_autopilot` delegates bootstrap (config/overlay/session initialization + workflow trigger injection) to `mi/runtime/wiring/bootstrap.py`, runs run-start seeding via `mi/runtime/wiring/run_start.py`, parses runtime feature flags via `mi/runtime/wiring/runtime_cfg.py`, wires checkpoint pipeline composition via `mi/runtime/wiring/checkpoints.py`, wires checkpoint mining/materialization callbacks via `mi/runtime/wiring/mining.py`, wires loop-guard/next-input queueing via `mi/runtime/wiring/next_input.py`, bootstraps segment buffer IO via `mi/runtime/wiring/segments.py`, centralizes Mind call circuit-break state via `mi/runtime/wiring/mind_call.py`, centralizes state-corruption warning flushing via `mi/runtime/wiring/state_warnings.py`, and composes the run-loop/pipeline/checkpoint execution via `RunLoopOrchestrator` (`mi/runtime/autopilot/orchestrator.py`). Thought DB prompt-context assembly and run-end WhyTrace candidate retrieval are routed through `ThoughtDbApplicationService` (`mi/thoughtdb/app_service.py`) as a shared facade. Phase semantics and user-visible loop behavior remain unchanged.
