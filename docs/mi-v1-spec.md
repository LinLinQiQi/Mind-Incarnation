...
Internal wiring (behavior-preserving): `mi/runtime/runner.py:run_autopilot` is the public entrypoint (thin wrapper). `mi/runtime/runner_core.py:run_autopilot` delegates bootstrap (config/overlay/session initialization + workflow trigger injection) to `mi/runtime/wiring/bootstrap.py`, then delegates post-boot wiring + run-loop composition to `mi/runtime/wiring/run_from_boot.py:run_autopilot_from_boot`. Run-level mutable state lives in `mi/runtime/runner_state.py` (`RunnerWiringState` + `RunnerStateAccess`) to reduce wiring drift (instead of many `nonlocal` vars / setter closures). Prompt builders live in `mi/runtime/prompts/` and are imported as `mi.runtime.prompts`. Most runner-specific wiring bundles live under `mi/runtime/wiring/bundles/` (batch_context, hands_runner, predecide, workflow_risk, preaction, decide, next_input, risk_predecide, testless, checkpoint_mining). Small, behavior-preserving helper builders (segment IO bootstrap, runtime-cfg normalization, MindCaller construction, decide_next logging builder, learn_suggested handler builder, segment add builder, cross-project recall writer builder, BatchPredecideDeps construction, checkpoint callbacks builder, run-end callback assembly) live in `mi/runtime/wiring/run_from_boot_builders.py`. Within `mi/runtime/wiring/run_from_boot.py`, phase wiring assembly is organized into a few small local helpers to keep drift low. The cross-project recall writer reads `thread_id` at call time (via getter) to keep provenance accurate. Phase assembly returns a small structured object (`PhaseAssembly`) rather than a tuple to reduce wiring drift. Dict-shaped inputs shared across bundles (overlay/workflow/runtime cfg) are normalized via `mi/runtime/wiring/phase_inputs.py` to reduce drift. The rest of the runtime loop wiring stays in `mi/runtime/wiring/` (e.g., `run_start.py`, `runtime_cfg.py`, `check_plan.py`, `auto_answer.py`, `ask_user.py`, `predecide_user.py`, `workflow_progress.py`, `decide_next.py`, `loop_break_checks.py`, `checkpoints.py`, `mining.py`, `next_input.py`, `segments.py`, `mind_call.py`, `state_warnings.py`). The run-loop orchestrator is built in `mi/runtime/composition.py` and executed via `RunLoopOrchestrator` (`mi/runtime/autopilot/orchestrator.py`). Small runner-only pure helpers live in `mi/runtime/runner_helpers.py` (best-effort coercions like `dict_or_empty`, and check-plan extraction like `get_check_input`). Thought DB prompt-context assembly and deterministic retrieval helpers are routed through `ThoughtDbApplicationService` (`mi/thoughtdb/app_service.py`) and shared building blocks in `mi/thoughtdb/predicates.py` (active/valid filters) + `mi/thoughtdb/retrieval.py` (seed ids + 1-hop expansion). CLI entrypoint remains `mi/cli.py:main` (argparse surface composed in `mi/cli_parser.py` + `mi/cli_parsers/`, behavior-preserving). Phase semantics and user-visible loop behavior remain unchanged.
