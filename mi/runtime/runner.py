...
def run_autopilot(
    *,
    ...
    # Optional: one WhyTrace at run end for auditability (opt-in; best-effort).
    if auto_why_on_end and executed_batches > 0 and last_batch_id:
        ...
        if isinstance(target_obj, dict) and ev_id:
            as_of_ts = now_rfc3339()
            ...
            if not kind:
                # EvidenceItem records omit `kind`.
                kind = "evidence_item"
            target = {
            ...
            if candidates:
                ...
                why_obj, why_ref, why_state = _mind_call(
            ...
            # Enforce: chosen_claim_ids must be a subset of candidate ids.
